...
func TestSlurmDepTrustsRecordedID(t *testing.T) {
	...
	if _, err := c.ClaimNextJob(ctx, "test-runner", "slurm", 0, 0, 0); err != nil {
...
func TestSlurmDepSkipsSucceeded(t *testing.T) {
	...
	if _, err := c.ClaimNextJob(ctx, "test-runner", "simple", 0, 0, 0); err != nil {
		t.Fatalf("ClaimNextJob: %v", err)
	}
...
func TestEndJobNotesPersisted(t *testing.T) {
	...
	if _, err := c.ClaimNextJob(ctx, "runner-x", "simple", 0, 0, 0); err != nil {
		t.Fatalf("ClaimNextJob: %v", err)
	}
