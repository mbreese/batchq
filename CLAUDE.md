...
- `[slurm_runner]` — `user`, `account`, `partition`, `max_jobs` (per-invocation submit cap; `--max-jobs` flag), `max_slurm_jobs` (cap on this user's live SLURM-queue jobs; `--slurm-max-jobs` flag), `min_array` (minimum array batch size — defer batches smaller than this unless it's the array's final remainder; `--slurm-min-array` flag; clamped to the effective job cap), `full_array` (all-or-nothing — only submit an array that fits a single pass entirely; `--slurm-full-array` flag; overrides `min_array`), plus `[slurm_runner.resources]` (typically cluster/feature labels so resource-tagged jobs route to the right cluster; procs/mem/walltime stay SLURM-enforced).
